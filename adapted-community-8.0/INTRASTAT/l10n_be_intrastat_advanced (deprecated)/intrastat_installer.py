...
class intrastat_installer(models.TransientModel):
    ...
    def _load_code(self, row):
        code_obj = self.env['report.intrastat.code']
        vals = {'description': row['description']}
        cn_unit_id = row['unit_id']
        if cn_unit_id:
            cn_unit_ref = 'l10n_be_intrastat_advanced.' + cn_unit_id
        ...
        if isinstance(cn_code_i, int):
            ...
            vals['intrastat_code'] = cn_code
            code_obj.create(vals)

    @api.multi
    def execute(self):
        res = super(intrastat_installer, self).execute()
        self.cn_codes = self.env['report.intrastat.code'].search([])
        self.cn_lookup = {}
        for i, c in enumerate(self.cn_codes):
            self.cn_lookup[c.intrastat_code] = i
        CN_fqn = 'l10n_be_intrastat_advanced/data/' \
